...
## Release tag scheme
- chippy ships under bare `vX.Y.Z` tags. Goreleaser via `.goreleaser.chippy.yml` (last released: `v1.12.0` — ADR 0015 65816 tail (epic #517): playground bank selector — `jsReadMem`/`jsDisasm` 24-bit + web `bank` input (#521); pinned cross-bank disassembly + `:da $BB:XXXX` command, `DisasmAnchorBank`/`disasmBank()` (#520); hardware-interrupt per-cycle bus trace — `serviceInterrupt816` emits the 2 leading internal cycles, validated by `cpu/interrupt816_test.go` against a hand-authored WDC reference (#519). Only #518 (ABORT) remains, parked (no abort source). v1.11.0 — ADR 0014 65816 tail: hardware interrupt handling (`serviceInterrupt816`, #511 — native/emulation IRQ/NMI vectors, PBR push, D-clear, bus24), remote 65816 bank reads over DAP (#510 — `RemoteSource` fetches banks >0 lazily over the wire), and 65816 in the WASM playground (#514) + native-mode demo `hello816.bin` (#515). v1.10.0 — ADR 0013 bank-aware 24-bit bus for the 65816 (`cpu.Banked24`, #505): kills the bank-0 mirror — bank 0 routes through the 16-bit MMIO/WBus/RAM chain, banks 1-255 get a flat 16 MB store; Intel HEX type-04 loads into banks >0, DAP/TUI bank-aware (`:bank N`, `$BB:XXXX`), plus cross-bank PBR-relative disassembly (#507). v1.9.0 — ADR 0012 accuracy tail: per-cycle 65816 bus trace (`TestHarte65816BusTrace`, #495) — chunks 2–4 + full pin-string validation (all 256 opcodes both modes, addr + value + 8 pin bits VDA/VPA/VPB/RWB/E/M/X/MLB, stricter than the 6502/65C02 traces; four skips: WAI/STP/MVN/MVP). v1.8.0 — ADR 0011 accuracy tail: 2A03 DMA-read open-bus seam (`DmaReadBus`, #481), then the DMC-DMA steal-timing fixes that close `dmc_dma_during_read4` / nessy #20 — `idle()` polls `ProcessPendingDma` so a halt drains on the taken-branch dummy-read cycle (4-cycle steal) + true-cycle `getCycle` (#493/#494/#497); plus 65816 per-cycle bus trace chunk 1 (#495). v1.7.0 — epic #458: full Tom Harte-validated WDC 65C816 core (#456, 256 opcodes, emulation + native), TUI-via-DAP render+control flip (#461/#471), freeze beyond RAM (#463), DAP data breakpoints + setVariable (#453/#454), 65C02 per-cycle bus trace (#455/#475), hosted WASM playground (#457). v1.6.0 — accuracy tail + debugger UX cleanup, epic #438: 238/238 6502 bus-exact, 65C02 Tom Harte, struct overlay watch, DAP array children, chippy-state dirtyRanges, goreleaser cask. v1.5.0 — DAP onramp + complete CPU ROM coverage, epic #402; host debug hooks, epic #419. nessy carved out into [github.com/nkane/nessy](https://github.com/nkane/nessy) post-v1.2.0; VS Code extension removed v1.4.1).
